...
public class MenuOwnerController {
    ...
    public ResponseEntity<OwnerMenuResponse> updateMenu(@PathVariable Long storeId,
                                                        @Parameter(hidden = true) @Auth AuthUser authUser,
                                                        @PathVariable Long menuId,
                                                        @Valid @RequestBody MenuUpdateRequest menuRequest) {
        OwnerMenuResponse menuResponse = menuOwnerService.updateMenu(storeId, authUser.getId(), menuId, menuRequest.getCategory(), menuRequest.getName(), menuRequest.getPrice(), menuRequest.getInfo());
        return new ResponseEntity<>(menuResponse, HttpStatus.OK);
    }

    @GetMapping("/{storeId}/menus")
    @Operation(summary = "메뉴 반환 - param에 menuId 입력 시 특정 메뉴 반환")
    public ResponseEntity<List<OwnerMenuResponse>> getMenus(@PathVariable Long storeId, @Parameter(hidden = true) @Auth AuthUser authUser,
                                                       @RequestParam(required = false) Long menuId) {
        List<OwnerMenuResponse> menus = menuOwnerService.getMenus(storeId, authUser.getId(), menuId);
